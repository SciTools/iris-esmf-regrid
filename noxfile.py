...
def _lockfile_path(
    py_string: str, platform_placeholder: bool = False, experimental: bool = False
) -> Path:
    """
    ...
    name_template = "{py_string}{experimental}-{platform}.lock"
    if platform_placeholder:
        platform = "{platform}"
    else:
        platform = LOCKFILE_PLATFORM
    if IRIS_SOURCE is not None or experimental:
        experimental = "-experimental"
    else:
        experimental = ""
    lockfile_name = name_template.format(
        py_string=py_string, experimental=experimental, platform=platform
    )
...
def _get_iris_github_artifact(
    session: nox.sessions.Session, experimental: bool = False
) -> str:
    """
    ...
    result = IRIS_SOURCE
    if experimental:
        # While we are testing against multiple versions of iris, we have to
        # define the iris source here also.
        result = "github:mesh-data-model"
    if not result:
    ...
    for req_file in Path("requirements").glob(r"py[0-9]*.yml"):
        for experimental in [True, False]:
            ...
            filename_template = _lockfile_path(
                python_string, platform_placeholder=True, experimental=experimental
            )
            lockfile_path = _lockfile_path(
                python_string, platform_placeholder=False, experimental=experimental
            )
