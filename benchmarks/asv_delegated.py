...
class Delegated(_DelegatedABC):
    ...
    def _prep_env_override(self, env_parent_dir: Path) -> Path:
        ...
        Scans the checked-out commit of IER to work out the appropriate
        ...
        if noxfile.is_file():
            ...
            #  `run_always` to `_prepare_venv` enabled environment setup without
            #  running tests.
            noxfile_tree = ast.parse(source=noxfile.read_text())
            prep_session = next(
                filter(
                    lambda node: getattr(node, "name", "") == "_prepare_env",
        ...
        match mode:
            ...
            case Mode.NOX:
                ...
                # Prevent Nox reusing env with wrong Python version.
                env_parent_dir = (
            ...
            case _:
                message = (
                    "No environment setup is known for this commit of Iris-esmf-regrid."
                )
